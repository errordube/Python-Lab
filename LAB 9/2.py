...
Label(root,text="Course:").grid(row=6,column=0)
...
Label(root,text="Student Email:").grid(row=11,column=0)
Label(root,text="CGPA:").grid(row=12,column=0)
Label(root,text="Hobbies:").grid(row=13,column=0)
Label(root,text="Certification(RHCE/MCSE/CCNA/OCP):").grid(row=14,column=0)
Label(root,text="Enter Roll Number to fetch:").grid(row=15,column=0)
...
def insert():
    ...
    if len(dob.get())<=7:
        showerror('Date of Birth','Not Valid')
    if len(s_email.get())<8:
        showerror('Email Id','Not valid')
    if len(course.get())<=0:
        showinfo('Course','Please write your course')
    if len(dept.get())<=0:
        showinfo('Department','Please select your department')
    if len(cgpa.get())<=0:
        showinfo('CGPA','Please check your CGPA')
    
def show():
    cur.execute("select * from stud where roll_number= "+(rid.get()))
    x=cur.fetchall()
    Label(root,text=x).grid(row=17,column=1)
